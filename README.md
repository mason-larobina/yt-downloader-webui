...
Items can be cancelled (pending removal or killing the active download), retried (re-enqueued at the back), or cleared (terminal items dropped). Done items dedupe: re-submitting a URL whose previous item is `done` is a no-op, so re-downloading a playlist doesn't create duplicate files. A per-item details page (`/item/:id`) shows the full thumbnail, metadata, and the complete yt-dlp log output (polled while the download may still be producing lines).
