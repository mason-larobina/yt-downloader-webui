...
paste a video URL into the header input, it gets **appended to a global
queue**, and a single background worker downloads it while you watch its
progress live in the browser. Pasting a playlist URL **probes** it (a
dedicated SSE stream runs `yt-dlp --flat-playlist -j`, extraction only, no
download) and presents the per-video entries as confirm cards for you to pick
from; a single video is enqueued directly. The queue and live status are
**shared across all sessions/tabs**: open the app in a second tab and you see
the same queue and the same active download. Runs against the user's own
home directory, using fresh cookies from the local Firefox profile by
default.
...
- One single-line URL input in the header. Submitting (or pasting anywhere on
  the page -- the input catches the paste and submits immediately) swaps in a
  **probe area** wired to its own `GET /probe?url=…` SSE stream that runs
  `yt-dlp --flat-playlist -j` (fast: extraction only, no download) and streams
  its output live. A **playlist** is expanded into its per-video entries and
  returned as a list of N files **to approve** (checkboxes, approve a subset);
  a **single video** is enqueued for download directly. Only approved
  per-video items (and directly-queued single videos) are **appended to a
  global queue** and **persisted to one JSON file per item** so they survive
  restarts (see Sec. 8) -- **playlists are never persisted**: their expansion
  lives only in the request that produced it. A single background worker
  drains the queue **one URL at a time**, spawning one `yt-dlp` process per
  item. There is only ever one yt-dlp *download* running at any moment (a
  probe may run concurrently in its own SSE stream -- it writes no files);
...
- A rich media library (transcoding, tagging, search). The library is a flat
  file list with open/download/delete; no metadata DB. (Per-video thumbnails
  *are* shown -- fetched during the probe or generated by ffmpeg after a
  download, then cached -- but there is no tagging/search. Queue *does*
...
with a clear, actionable message (install yt-dlp / fix `--yt-dlp`). It also
soft-checks `ffmpeg -version`; a missing ffmpeg is only a `warn!` (thumbnail
generation is skipped, not fatal).
...
home as reported by the `dirs` crate). The resolved path is created if
missing. The `--state-dir` default (`~/.local/share/yt-downloader-webui/queue/`)
and the `--cache-dir` default (`~/.cache/yt-downloader-webui/thumbs/`) are
resolved the same way; both directories are created at startup.
...
- `--cookies-from-browser firefox` -- reads fresh cookies from the home Firefox
  ...
  profile" requirement with zero configuration. Omitted entirely when
  `--cookies-from-browser none` is passed.
...
- `--progress-template '%(progress)j'` -- replaces the human progress bar with a
  ...
  `total_bytes_estimate`, `speed`, `eta`, `elapsed`, plus yt-dlp's preformatted
  `_percent`/`_percent_str`, `_speed_str`, `_eta_str`, etc.
...
- `-f '...'` -- format selector that prefers mp4 video ≤1080p + m4a audio,
  falling back through progressively looser alternatives (any-height mp4+m4a,
  single progressive mp4 ≤1080p, single progressive mp4 any height, then best
  anything as a last resort) so the download always grabs *something* rather
  than failing on an unusual source.
- `--merge-output-format mp4` -- when the best video and audio arrive as
  separate streams, the merged container is mp4 even if the picked streams
  are not mp4-native. yt-dlp then emits a `[Merger] Merging formats into
  "<path>"` line naming the final file (see Sec. 5).
...
The child is built with `kill_on_drop(true)`, so a dropped worker future
(shutdown) reaps the process automatically; `POST /cancel` kills it explicitly.
...
Classification (playlist vs. single video) happens on a *separate* command
...
`--flat-playlist -j` prints one JSON line per playlist entry (a playlist) or
one big video dict (a single video). No `-P`, no `--progress-template` -- this
process writes no files and emits no progress ticks. On a non-playlist URL
`--flat-playlist` is a no-op (yt-dlp fully extracts the video), so a
single-video paste costs one extraction here plus one at download time; this
is accepted for robustness (no URL-shape guessing). Cookies are reused so
YouTube (which 403s anonymous, cookie-less requests) responds. The probe is
`kill_on_drop(true)` and bounded by a 60 s timeout (Sec. 6).

---

## 5. Parsing yt-dlp output

stdout/stderr are read line-by-line via `tokio::io::BufReader`. Two line
shapes are parsed:

- **Download phase** (`parse_line`): a line that parses as a JSON object
  containing a `status` field is a **progress event** -> update the active
  queue item; everything else (yt-dlp status lines, `WARNING:`, `ERROR:`,
  non-status JSON) is a **log event**.
- **Probe phase** (`parse_flat_line`): a line that parses as a JSON object is a
  `--flat-playlist -j` entry (`_type:"url"` for a playlist entry, a full video
  dict for a single video); non-JSON lines are probe log lines (and the last
  `ERROR:` line is harvested as the surfaced error).

| Download-phase line shape                                  | Handling                                         |
|--------------------------------------------------------|--------------------------------------------------|
| JSON object with a `status` field                       | **Progress event** -> update active item's `progress`. |
| `[download] Destination: <file>`                         | Set item's `filename` (basename) if unset.      |
| `[download] <file> has already been downloaded`        | Set `filename` (yt-dlp emits no progress/`Destination:` for a cache hit -- this is the only source of the name). |
| `[Merger] Merging formats into "<file>"`                | **Override** `filename` with the final merged name (the authoritative last name after per-stream temp files). |
| Progress `status == "finished"` / exit code 0          | Mark active item `Done`; worker pops next.       |
| Progress `status == "error"` / non-zero exit / `ERROR:` line | Mark `Failed`, harvest the `ERROR:` message into `item.error` (last wins; most specific). |
| Anything else (stderr `WARNING:` included)               | **Log event** -> push into `log_ring` + the active item's per-item `logs`, emit a `log` event. |

The probe phase calls `parse_flat_line` directly (it never sees progress
JSON). Each `FlatEntry` captures only `_type`, `url`, `title`, `duration`,
and the best `thumbnail` URL (highest `width*height` from the entry's
`thumbnails` array, falling back to the top-level `thumbnail` string for a
single-video dict) -- everything else in the JSON is dropped.
...
item's percent/speed/ETA; plain-text log lines populate the per-item log
buffer (`item.logs`, capped to 2000 lines, persisted with the item) and the
global `log_ring` replayed to newly-connected tabs (see Sec. 8).

### Throttling

yt-dlp emits progress ticks roughly 10-20x/s. The server **throttles** emit to
at most one every 200 ms (always emitting the final `finished`/`error` state).
Only the **banner** (`status` event) updates on every throttle tick -- it
carries the live progress bar / percent / ETA. The cards list is **not**
re-rendered each tick (doing so would destroy/recreate every card's DOM every
200 ms, re-triggering the overlay fade-in and dropping `:hover` state); a
`queue` swap is emitted only when a card-visible field actually changes
(filename/label, error, status transition), plus a final `queue` swap on item
termination.
