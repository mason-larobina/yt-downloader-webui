...
| `endpoints.sh`               | Queue controls (`/download` -> probe-area shell, GET `/probe` SSE result, POST `/confirm`, `/cancel`, `/retry`), file serving (inline/download/range), `/delete`, path-traversal guards, probe-error -> result event (no queue item), cancel+retry cycle. |
...
- `endpoints.sh` uses a nonexistent YouTube video ID to exercise the probe
  failure path (error returned as a `result` event with an error + Done
  button, nothing enqueued); it also probes+confirms a real archive.org
  download to drive the cancel+retry cycle. It needs no cookies.
